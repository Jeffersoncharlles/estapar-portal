...
const dbGarages = [...garagesData]
const dbGarageDetails = [...garageDetailsData]
const dbGaragePlans = readGaragePlansFromStorage() ?? [...garagePlansData]
...
export const handlers = [
	...
	http.put("*/api/planos", async ({ request }) => {
		...
		const planIndex = dbGaragePlans.findIndex(
			(plan) =>
				plan.garageId === originalPlan?.garageId &&
				plan.description === originalPlan?.description &&
				plan.vehicleType === originalPlan?.vehicleType &&
				Number(plan.totalSpotsByVehicleType) ===
					Number(originalPlan?.totalSpotsByVehicleType) &&
				Number(plan.price) === Number(originalPlan?.price) &&
				Number(plan.cancellationFee) ===
					Number(originalPlan?.cancellationFee) &&
